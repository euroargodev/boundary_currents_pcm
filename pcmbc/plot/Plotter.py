"""
pyXpcm output visualisation helper class

Based on class developed within [BlueCloud project](https://github.com/euroargodev/BlueCloud)
"""
...
class Plotter:
    """Helper class for visualisation of data from pyxpcm
    ...
    Examples
    --------
    P = Plotter(dsi, m, coords_dict={'latitude': 'LATITUDE', 'longitude': 'LONGITUDE', 'time': 'TIME', 'depth': 'DEPTH_INTERPOLATED'}, cmap_name='Set2')
    P.pie_classes()
    ...
    P.plot_robustness(lon_grid=1, lat_grid=1)
    P.vertical_structure(q_variable='TEMP_Q', sharey=True, ylim=[-1000, 0], figsize=(10,5))
    P.vertical_structure_comp(q_variable='TEMP_Q', sharey=True, ylim=[-1000, 0], figsize=(10,5))
    """
    ...
    def vertical_structure(self,
       ...
       **kwargs
       ...
       ax : :class:`matplotlib.axes.Axes` object or array of Axes objects.
        ...
        # fig.text((fig_size[0]/2)/fig_size[0], 0.35, xlabel, va='center', fontsize=10)
        ax[0].set_xlabel(xlabel, va='center', fontsize=10)
        # plt.tight_layout()
    ...
    def vertical_structure_comp(self, q_variable,
        ...
        fig, ax = self.m.plot.subplots(maxcols=maxcols, K=nQ_p, **{**defaults, **kwargs})
    ...
    def spatial_distribution(self, proj=ccrs.PlateCarree(), extent='auto', time_slice=0, lon_grid = 4, lat_grid = 4, **kwargs):
        ...
        defaults = {'figsize': (6, 6), 'dpi': 120,
                    'facecolor': 'w', 'edgecolor': 'k'}
        fig, ax = plt.subplots(nrows=1, ncols=1, subplot_kw=subplot_kw, **{**defaults, **kwargs})
        kmap = self.m.plot.cmap(name=self.cmap_name)
    ...
    def plot_posteriors(self, proj=ccrs.PlateCarree(), extent='auto', time_slice=0, maxcols=3, lon_grid=4, lat_grid=4, **kwargs):
        ...
        defaults = {'figsize': (wi, hi), 'dpi': 120,
                    'facecolor': 'w', 'edgecolor': 'k'}
        fig, ax = self.m.plot.subplots(maxcols=maxcols, subplot_kw=subplot_kw, **{**defaults, **kwargs})

        for k in self.m:
            if self.data_type == 'profiles':
                sc = ax[k].scatter(dsp[self.coords_dict.get('longitude')], dsp[self.coords_dict.get('latitude')], s=3, c=dsp['PCM_POST'].sel(pcm_class=k),
