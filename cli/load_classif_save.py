...
def load_and_classify(this_m, this_df):
    with warnings.catch_warnings():
        ...
        try:
            ds = (
                DataFetcher(src="erddap", mode="expert", cache=False)
            ...
            try:
                ...
                with warnings.catch_warnings():
                    ...
                    dsi_float = from_misc_pres_2_std_depth(
                        this_m, dsp_float, feature_name="temperature"
                    )
                    std = True
                ...
                this_m.predict(
                    dsi_float,
                    features={"temperature": "TEMP", "salinity": "PSAL"},
                    dim="DEPTH_INTERPOLATED",
                    inplace=True,
                )
                ...
                if std:
                    msg = (
                        "Data downloaded but can't be classified: PCM prediction failed"
                    )
                else:
                    msg = "Data downloaded but can't be classified: data can't be projected on standard depth levels"
                print(
                    "Error with WMO=%i, CYC=%i: %s"
                    % (this_df["wmo"].values[0], this_df["cycle_number"].values[0], msg)
                )
            ...
            try:
                print(
                    "Error with WMO=%i, CYC=%i: %s"
                    % (
                        this_df["wmo"].values[0],
                        this_df["cycle_number"].values[0],
                        argopy.dashboard(
                            wmo=this_df["wmo"].values,
...
# Date of update : {}
""".format(
        BCname, pd.to_datetime("now", utc=True).strftime("%Y%m%d%H%M%S")
    )
...
if __name__ == "__main__":
    ...
    index["cycle_number"] = index.apply(
        lambda x: int(
            "".join(
                [
                    c
                    for c in x["file"].split("/")[-1].split("_")[-1].split(".nc")[0]
                    if c.isdigit()
                ]
            )
        ),
    ...
    for prof, df in index.groupby(["wmo", "cycle_number"]):
        ...
        try:
            df["url"] = argopy.dashboard(
                wmo=df["wmo"].values, cyc=df["cycle_number"].values, url_only=True
            )
            index.loc[
                np.logical_and(index["wmo"] == wmo, index["cycle_number"] == cyc)
            ] = df
        except:
            pass
    ...
    for prof, df in tqdm(index.groupby(["wmo", "cycle_number"])):
        ...
        index.loc[np.logical_and(index["wmo"] == wmo, index["cycle_number"] == cyc)] = (
            df_updated
        )
    ...
    index["reordered_label"] = index.apply(
        lambda x: (
            korder[int(x["pcm_label"])]
            if (not np.isnan(x["pcm_label"]) and x["pcm_label"] < 999)
            else np.nan
        ),
        axis=1,
    )
    index["reordered_label"] = index.apply(
        lambda x: (
            999
            if (np.isnan(x["reordered_label"]) and x["pcm_label"] >= 999)
            else x["reordered_label"]
        ),
        axis=1,
    )
    ...
    csvfile = os.path.join(
        out_dir,
        "BC_%s_index_classified.txt" % BCname.replace(" ", "_").replace(".", ""),
    )
    ...
    for k in m:
        ...
        print(
            "%i profiles in k=%i (%s)" % (n, korder[k], class_descriptions[korder[k]])
        )
        yticklabels[korder[k]] = "Class #%i:%s\n%i profiles" % (
            korder[k],
            class_descriptions[korder[k]],
            n,
        )
    ...
    subplot_kw = {
        "projection": proj,
        "extent": np.array(box) + np.array([-dx, +dx, -dy, +dy]),
    }
    fig, ax = plt.subplots(
        nrows=1,
        ncols=1,
    ...
    if aviso_nrt:
        ...
        aviso_nrt["adt"].plot.contour(
            levels=np.arange(-2, 2, 0.1), ax=ax, colors="gray", zorder=0, linewidths=0.1
        )

    classified = index.where(
        np.logical_and(
            index["reordered_label"] < 999, ~np.isnan(index["reordered_label"])
        )
    )
    sc = ax.scatter(
        classified["longitude"],
        classified["latitude"],
        s=46,
        c=classified["reordered_label"],
        cmap=m.plot.cmap(name=cname),
        transform=proj,
        vmin=0,
        vmax=m.K,
        edgecolors="k",
    ...
    if aviso_nrt:
        ax.text(
            ...
            % pd.to_datetime(aviso_nrt["adt"]["time"].values).strftime(
                "%Y/%m/%d %H:%M"
            ),
            color="gray",
            fontsize=10,
    ...
    out_name = os.path.join(
        *[out_dir, os.path.split(index_file)[1].replace(".txt", "_classified.png")]
    )
    print(out_name)
